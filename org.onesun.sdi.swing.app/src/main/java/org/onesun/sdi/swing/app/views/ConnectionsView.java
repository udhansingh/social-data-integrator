...
import org.onesun.sdi.swing.app.api.ConnectionView;
import org.onesun.sdi.swing.app.api.ConnectionViewsFactory;
...
public class ConnectionsView extends JPanel {
	...
	class NodeRenderer extends DefaultTreeCellRenderer {
		...
		public Component getTreeCellRendererComponent(
			...
			if (leaf) {
				if(object instanceof OAuthConnection){
					OAuthConnection c = (OAuthConnection)object;
					setText(c.getName());
	...
	private class ConnectionTreeSelectionListener implements TreeSelectionListener{
		...
		private void renderConnectionDetailView(String connectionName){
			...
			Connection connection = ConnectionsFactory.getConnectionByName(connectionName);
			AppCommons.TASKLET.setConnectionProperties(connection);
			AppCommons.AUTHENTICATION = null;

			if(connection == null){
				container.add(ConnectionViewsFactory.DEFAULT_CONNECTION_VIEW, BorderLayout.CENTER);
			}
			else {
				...
				if(category != null && category.trim().length() > 0){
					ConnectionView view = ConnectionViewsFactory.getViewByCategory(category);
	...
	private void initModel(){
		Map<String, Connection> connections = ConnectionsFactory.getConnections();
				...
				public void actionPerformed(ActionEvent e) {
					...
					if(object != null){
						if(object instanceof SocialMediaProvider){
							view = ConnectionViewsFactory.newConnectorViewByName("Social Media");
							
							SocialMediaProvider p = (SocialMediaProvider)object;

							if(view instanceof SocialMediaConnectionView){
								SocialMediaConnectionView v = (SocialMediaConnectionView)view;
								v.init();
							...
							ServiceProvider p = (ServiceProvider)object;
							view = ConnectionViewsFactory.newConnectorViewByCategory(p.getCategory());
							view.init();
						}
						...
						if(view != null){
							...
							ConnectionViewPanel v = view.getView();
							if(v != null){
