 ...
 */
package org.onesun.sdi.sil.auth;
...
public class OAuthenticator {
	...
	private static String 		CALLBACK_CONTEXT 	= "/ows/jrs/callback";
	...
	private volatile OAuthConnection connection = null;
	...
	public OAuthenticator(SocialMediaProvider provider, OAuthConnection connection, final int timeout){
		this(provider, connection);

		OAuthenticator.TIMEOUT = timeout;
	}

	public OAuthenticator(SocialMediaProvider provider, OAuthConnection connection){
	...
	public boolean getAccessKeys(String url){
		...
		OAuthVersion oauthVersion = provider.getOAuthVersion();

		if(oauthVersion == OAuthVersion.VERSION_2){
		...
		if(service != null && verifier != null && accessToken == null && invoked == false) {
			invoked = true;
			if(oauthVersion == OAuthVersion.VERSION_2){
